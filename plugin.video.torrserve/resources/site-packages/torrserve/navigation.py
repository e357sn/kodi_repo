...
def root():
    resp = client("/echo")
    if not resp:
        notify(translate("Server %s is down") % TORRSERVED_HOST)
        listing = get_listing("noindex")
        return Plugin.create_listing(listing, category="TorrServer: N/A")
    else:
...
def search(params):
    ...
    if params.mod == "index":
        ...
        if query:
            yohoho_query = query.replace(" ", "%20")
            result = client("/", get_data={'title': yohoho_query}, host='http://4h0y.yohoho.cc')
            if not result:
            ...
            return Plugin.create_listing(listing, category="{0}: {1}".format(translate("Search"), query), sort_methods=(2, xbmcplugin.SORT_METHOD_SIZE), cache_to_disk=True, content="videos")
    return False
...
def play_now(params):
    ...
    while True:
        ...
        if len(stat['FileStats']) > 1 and selFile is None:
            ...
            sel = sDialog.select(translate('Choose a file'), filelist)
            if sel == -1:
...
def makeFileList(hash):
    response = client("/torrent/list", post_data={"Request": 1})
    listing = []
    for t in response:
        if t['Hash'] == hash :
            for f in t['Files']:
...
def makePreloadDialog(hash):
    pDialog = xbmcgui.DialogProgress()
    pDialog.create("TorrServer", translate("Wait for info..."), translate("Connected: %d | Active: %d | Total: %d") % (0, 0, 0))
    success = False
    counter = 0
    while True:
        if pDialog.iscanceled() :
        ...
        line2 = translate("Connected: %d | Active: %d | Total: %d") % (stat['ConnectedSeeders'], stat['ActivePeers'], stat['TotalPeers'])
